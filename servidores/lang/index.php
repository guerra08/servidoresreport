...
echo html_writer::input($instanceoptions, 'instanceid', $instanceid);
...
*/
	/* NOSSO SQL */
	$sql = "SELECT cmc.id, cmc.userid, u.firstname, u.lastname, cmc.timemodified 
			FROM {user} u, {course_modules_completion} cmc, {course_modules} cm, {modules} m
			WHERE cmc.timemodified BETWEEN 1494979200 and 1495670400 AND
			cmc.completionstate = '1' and  m.name = 'checklist' and cm.course = '126'
...
/*
	...
	$inicio = 1494979200; //17/05/2017 usei para testar se o if funciona
	$fim = 1495670400; // 25/05/2017 usei para testar se o if funciona*/

    foreach ($users as $u) {
		$data = array ($u->firstname, $u->lastname, date('d-m-Y', $u->timemodified));
			//print_r($data);
		/*
